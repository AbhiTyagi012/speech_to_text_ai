...
class WhisperSingleControl(ft.UserControl):
    ...
    @property
    def histogram_result(self):
        """Histogram result"""
        return self._histogram_result
    ...
    def translate_text(self, value: str):
        self._translate_result = value
        self.translate_text_field.value = value
    ...
    @histogram_result.setter
    def histogram_result(self, value: str):
        self._histogram_result = value
        self.histogram_result_field.value = value
        if self._histogram_result:
            self.copy_button.disabled = False
        else:
            self.copy_button.disabled = True
        self.update()
    ...
    def _build_controls(self):
        ...
        self.translate_text_field = self._build_translate_text_field()
        self.histogram_result_field = self._build_histogram_result_field()
    ...
    def build(self):
        return ft.Column(
            [
                ...
                ft.Row([self.histogram_result_field], expand=True),
                ft.Container(
                    content=ft.Column(
                        [
                            ft.Row(
                ...
                ft.Row([self.translate_text_field], expand=True),
            ]
        )
    ...
    def _build_result_text_field(self):
        return ft.TextField(
            ...
            expand=True,
            min_lines=40,
            hint_text="Transcribed Result",
            read_only=True,
        )
        
    def _build_translate_text_field(self):
        return ft.TextField(
            multiline=True,
            expand=True,
            min_lines=40,
            hint_text="Translated  Result",
            read_only=True,
        )
    ...
    def translate_result(self, target_languages=('en', 'hi')): 
        translator = Translator()
        translated_results = {}

        for lang in target_languages:
            translation = translator.translate(self.result, dest=lang)
            translated_results[lang] = translation.text

        result_string = f"English: {translated_results['en']}\nHindi: {translated_results['hi']}"
        return result_string
    
    # def histogram_test(self):
    #     return ft.Image(
    #     src="C:\\Users\\Abhi Tyagi\\Desktop\\speech-to-text-ai\\whisper_ui\\tab_views\\Audio_Waveform_Visualization_Effect_After_Effects.gif",
    #     width=1000,
    #     height=200,
    #     fit=ft.ImageFit.COVER
    # )
    ...
    def partial_result_received(self, partial_result: str, time_processed: str):
        if self.result == "":
            ...
            self.translate_text = self.translate_result()
            # self.histogram_result = self.histogram_test()
